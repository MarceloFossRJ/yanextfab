...
- [x] 5.4 Push a test commit with an `## Unreleased` entry to a throwaway branch merged into `main` (or exercise the workflow via `workflow_dispatch`/a fork) and confirm the release PR opens with the expected `VERSION` and `CHANGELOG.md` diff. Verified against the live repo: merging this change's own PR (#3) triggered `release.yml` successfully, but surfaced a real bug — the "tag if VERSION changed" step compared `HEAD^`..`HEAD`, which also matches the *first-ever* introduction of `VERSION` by any ordinary PR, not just a release-PR merge. It fired on PR #3's merge and pushed tag `v0.1.0` without going through the Release PR gate. Fixed in `.github/workflows/release.yml`: the tag step now also requires the push's commit message (or, for a merge-commit-strategy merge, either parent's) to identify it as the `release/next` branch landing (`"Release: v..."` subject, covering squash/rebase merges, or a `*release/next*` subject, covering the default merge-commit strategy) - validated by replaying the fix's logic against PR #3's actual merge commit locally and confirming it now correctly does *not* tag. Since `## Unreleased` was empty in that same push, `compute-release.py --check` correctly reported no pending release and skipped the PR-open steps, as expected. The still-open item from this finding: the `v0.1.0` tag that already landed on the remote from the buggy run - left in place pending user decision (see chat).
